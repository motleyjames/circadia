...
- iOS project lives in `phone/`. `npm run put-on-phone` is the Mac cable path (packs, `cap sync`, opens Xcode). This Linux VM cannot archive or codesign. Do not run `dock` / `put-on-dock` / `put-on-phone` here. Simulator is not the phone path.
