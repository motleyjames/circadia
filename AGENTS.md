...
- iOS project lives in `phone/`. `npm run put-on-phone` packs the Mac diary, signs with an existing development profile or an Xcode Accounts team, then `xcodebuild` + `native-run` onto a reachable physical iPhone. A keychain certificate is not an Xcode account. This Linux VM cannot archive or codesign. Do not run `dock` / `put-on-dock` / `put-on-phone` here.
