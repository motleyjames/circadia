...
- Local HTTP is hostile input. Any web page the user has open can send a no-preflight POST to 127.0.0.1 while Circadia runs. `/api/study` on both servers refuses a foreign `Origin` (`isLocalRequest` in Next, `localOrigin` in `electron/static-server.cjs`). A stored pack's filename comes from a UUID-checked participant id — never a raw `slice()`. `electron/static-server.cjs` must not throw out of its request handler: an uncaught error there ends the process and the Dock app dies (`GET /%` did exactly that). Served paths are checked after `realpath`, not only lexically.
