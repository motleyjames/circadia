...
This module uses Claude to review code changes for quality,
...
4. **Best Practices**: TypeScript/Go/React/Python patterns
5. **Integration**: Will this work in production?
...
"""
    if model is None:
        model = get_default_model("code_review")
...
def review_directory(
    directory: Path,
    extensions: List[str] = ['.ts', '.tsx', '.js', '.jsx', '.go', '.py'],
    context: str = "",
    max_files: int = 20,
) -> CodeReview:
    """
...
if __name__ == "__main__":
    ...
    test_file = Path(__file__).parent.parent / "core" / "llm_caller.py"
