...
describe("touch targets, confirms, and fault screens", () => {
  ...
  it("sets 44px min targets on coarse pointers and honors reduced motion", () => {
    ...
    expect(css).toContain("brand-open-recede");
    expect(css).toContain("brand-open-scrim");
    ...
    expect(css).not.toContain("countdown-orb-debut");
    ...
    expect(css).toContain(":has(.brand-open-cover");
