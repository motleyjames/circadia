...
describe("phone diary shell", () => {
  it("is version 0.8.23 and keeps the vault key local:this-computer", () => {
    expect(APP_VERSION).toBe("0.8.23");
    expect(JSON.parse(readFileSync("package.json", "utf8")).version).toBe("0.8.23");
  ...
  it("gives Tonight and the inner pages air under Ask, and hides the Mac install hint on a phone", () => {
    ...
    expect(readFileSync("src/lib/diary-shell.ts", "utf8")).toContain("export const OPEN_HOLD_MS = 800");
    expect(readFileSync("src/lib/diary-shell.ts", "utf8")).toContain("export const OPEN_COVER_MS = 2200");
    expect(readFileSync("src/lib/diary-shell.ts", "utf8")).toContain("export const OPEN_IDENTITY_MS = 3600");
  ...
  it("ships an iPhone diary shell that is not the Operator", () => {
    ...
    expect(readFileSync("phone/ios/App/App.xcodeproj/project.pbxproj", "utf8")).toContain("MARKETING_VERSION = 0.8.23");
    expect(readFileSync("phone/ios/App/App.xcodeproj/project.pbxproj", "utf8")).toContain("CURRENT_PROJECT_VERSION = 33");
    ...
    expect(readFileSync("src/components/brand-stage.tsx", "utf8")).toContain("APP_VERSION");
    expect(readFileSync("src/components/brand-stage.tsx", "utf8")).toContain("brand-open-build");
  ...
  it("put-on-phone signs from this Mac's cert, installs onto a reachable iPhone, and refuses Linux", () => {
    ...
    // Every local write route refuses a cross-site caller.
    for (const route of ["study", "fold-inbox", "locked-diary", "vault"]) {
      expect(readFileSync(`src/app/api/${route}/route.ts`, "utf8")).toContain("isLocalRequest");
    }
    // The stored-pack filename is built from a UUID-checked id, never raw input.
    expect(readFileSync("electron/static-server.cjs", "utf8")).toContain("UUID_RE.test(raw.participantId)");
    expect(readFileSync("electron/static-server.cjs", "utf8")).not.toContain(
      'typeof raw.participantId === "string" ? raw.participantId.slice(0, 8)',
    );
    ...
    expect(run.stdout).toContain("0.8.23");
