...
describe("phone diary shell", () => {
  it("is version 0.8.21 and keeps the vault key local:this-computer", () => {
    expect(APP_VERSION).toBe("0.8.21");
    expect(JSON.parse(readFileSync("package.json", "utf8")).version).toBe("0.8.21");
  ...
  it("gives Tonight and the inner pages air under Ask, and hides the Mac install hint on a phone", () => {
    ...
    expect(readFileSync("src/lib/diary-shell.ts", "utf8")).toContain("export const OPEN_HOLD_MS = 600");
    expect(readFileSync("src/lib/diary-shell.ts", "utf8")).toContain("export const OPEN_COVER_MS = 1400");
    expect(readFileSync("src/lib/diary-shell.ts", "utf8")).toContain("export const OPEN_IDENTITY_MS = 3100");
  ...
  it("ships an iPhone diary shell that is not the Operator", () => {
    ...
    expect(readFileSync("phone/ios/App/App.xcodeproj/project.pbxproj", "utf8")).toContain("MARKETING_VERSION = 0.8.21");
    expect(readFileSync("phone/ios/App/App.xcodeproj/project.pbxproj", "utf8")).toContain("CURRENT_PROJECT_VERSION = 31");
  ...
  it("put-on-phone signs from this Mac's cert, installs onto a reachable iPhone, and refuses Linux", () => {
    ...
    expect(run.stdout).toContain("0.8.21");
