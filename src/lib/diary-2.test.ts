...
describe("diary 2.0 invariants", () => {
  ...
  it("CONSENT_VERSION is 5 and a person who accepted 3 sees the screen again", () => {
    ...
    expect(hasCurrentConsent({ consentVersion: 4 })).toBe(false);
    expect(hasCurrentConsent({ consentVersion: CONSENT_VERSION })).toBe(true);
  });
});
