...
and §"Authentication" of `sync-spec-3.0.md`. Everything else in 3.0 stands, including the
Worker contract, which needs no change.
...
`GET /vault/{lookupId}` returns a small **account record**: a few hundred bytes of
ciphertext, encrypted under `lookupKey`, containing
...
`accountId`, `vaultSalt` and `authKey` are all **random at account creation and never
recomputed**.
...
**The Worker needs no change.** It stores bytes for an id and checks a hash. That it now
serves two kinds of object is invisible to it, which is the point of keeping it dumb.
...
2. `accountId`, `vaultSalt` and `authKey` are random at creation and never recomputed.
...
1. The Worker — already specified, needs no change, can ship first and be curl-tested.
